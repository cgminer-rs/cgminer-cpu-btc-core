...
/// 检测CPU特性并启用相应优化
fn detect_and_enable_cpu_features(target_os: &str, target_arch: &str) {
    println!("cargo:warning=🔍 Detecting CPU features");
    ...
    match target_arch {
        "x86_64" => {
            ...
            if is_feature_available("avx2") {
                ...
                println!("cargo:warning=🚀 AVX2 support detected");
            }
            
            if is_feature_available("sha") {
                println!("cargo:rustc-cfg=has_sha_ext");
                println!("cargo:warning=🔐 SHA extensions detected");
            }
            
            if is_feature_available("aes") {
                println!("cargo:rustc-cfg=has_aes_ni");
                println!("cargo:warning=🔒 AES-NI support detected");
            }
            
            if is_feature_available("bmi2") {
                println!("cargo:rustc-cfg=has_bmi2");
                println!("cargo:warning=⚡ BMI2 support detected");
            }
        }
        "aarch64" => {
            // ARM64特性检测
            ...
            if target_os == "macos" {
                ...
                println!("cargo:rustc-cfg=has_apple_sha");
                println!("cargo:warning=🍎 Apple Silicon crypto acceleration detected");
            } else {
                // 通用ARM64特性
                println!("cargo:rustc-cfg=has_crypto_ext");
                println!("cargo:rustc-cfg=has_sha2_ext");
                println!("cargo:warning=🦾 ARM64 crypto extensions detected");
            }
        }
        _ => {
            println!("cargo:warning=❓ Unknown architecture: {}", target_arch);
        }
    }
}
...
fn is_feature_available(feature: &str) -> bool {
    // 这里可以实现更复杂的CPU特性检测
    // 目前返回true，假设现代CPU都支持这些特性
    match feature {
        "avx2" | "sha" | "aes" | "bmi2" => true,
        _ => false,
    }
}
